...
// TODO:  Unit tests with files containing the EOF marker 

TEST(binfileTests, midiFileTestCSIO) {
	std::string fn {"..\\..\\dbklib_test\\test_materials\\test.mid"};
	auto bf = dbk::read_binary_csio(fn);

	int ans_file_nbytes = 4946;
	EXPECT_EQ(bf.size(),ans_file_nbytes);
	...
	for (int i=0; i<offset2656.size(); ++i) {
		EXPECT_EQ(bf[2656+i],offset2656[i]);
	}
	...
	for (int i=0; i<last18.size(); ++i) {
		EXPECT_EQ(bf[ans_file_nbytes-last18.size()+i],last18[i]);
	}
}

TEST(binfileTests, midiFileTestIOS) {
	std::string fn {"..\\..\\dbklib_test\\test_materials\\test.mid"};
	auto bf = dbk::read_binary_ios(fn);
...
TEST(binfileTests, midiFileTestWin) {
	std::string fn {"..\\..\\dbklib_test\\test_materials\\test.mid"};
	auto bf = dbk::read_binary_win32(fn);
...
TEST(binfileTests, emptyFileNoExtensionCSIO) {
...
TEST(binfileTests, emptyFileNoExtensionIOS) {
	std::string fn {"..\\..\\dbklib_test\\test_materials\\empty_file"};
	auto bf = dbk::read_binary_ios(fn);
...
TEST(binfileTests, emptyFileTxtExtensionCSIO) {
...
TEST(binfileTests, emptyFileTxtExtensionIOS) {
	std::string fn {"..\\..\\dbklib_test\\test_materials\\empty_file.txt"};
	auto bf = dbk::read_binary_ios(fn);
...
TEST(binfileTests, txtFileCSIO) {
	...
	for (int i=0; i<contents.size(); ++i) {
		EXPECT_EQ(contents[i],bf[i]);
	}
}


TEST(binfileTests, txtFileIOS) {
	std::string fn {"..\\..\\dbklib_test\\test_materials\\txt_file.txt"};
	auto bf = dbk::read_binary_ios(fn);
