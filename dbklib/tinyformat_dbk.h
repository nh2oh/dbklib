...
namespace tfm = tinyformat; // Namespace alias to encourage brevity
...
struct convertToInt	{
	static int invoke(const T& /*value*/) {
		assert(0 && "tinyformat: Cannot convert from argument type to "
...
class FormatArg	{
	...
	template<typename T>
	FormatArg(const T& value) : m_value(static_cast<const void*>(&value)),
...
public:
	...
	FormatListN(const Args&... args): FormatList(&m_formatterStore[0], N),
...
inline const char* streamStateFromFormat(std::ostream& out, bool& spacePadPositive,
	...
	if (*fmtStart != '%') {
		assert(0 && "tinyformat: Not enough conversion specifiers in format string");
		return fmtStart;
	}
	...
	if (*c == '*') {
		...
		if (argIndex < numFormatters) {
			...
			assert(0 && "tinyformat: Not enough arguments to read variable width");
		}
		if (width < 0) {
	...
	if (*c == '.') {
		...
		if (*c == '*') {
			...
			if (argIndex < numFormatters) {
				...
				assert(0 && "tinyformat: Not enough arguments to read variable precision");
			}
		} else {
			if (*c >= '0' && *c <= '9') {
	...
	case 'g':
		...
		break;
	case 'a': case 'A':
		assert(0 && "tinyformat: the %a and %A conversion specs "
			"are not supported");
		break;
	case 'c':
	...
	case 'n':
		...
		assert(0 && "tinyformat: %n conversion spec not supported");
		break;
	case '\0':
		assert(0 && "tinyformat: Conversion spec incorrectly "
...
inline void formatImpl(std::ostream& out, const char* fmt,
	...
	for (int argIndex = 0; argIndex < numFormatters; ++argIndex) {
		...
		if (argIndex >= numFormatters) {
			...
			assert(0 && "tinyformat: Not enough format arguments");
			return;
		}
	...
	fmt = printFormatStringLiteral(out, fmt);
	if(*fmt != '\0') {
		assert(0 && "tinyformat: Too many conversion specifiers in format string");
	}
